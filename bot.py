...
# ========== بررسی اتمام راند ==========
def is_round_complete(chat_id: int) -> bool:
    g = games.get(chat_id)
    if not g:
        return False
    for cat in CATEGORIES:
        for uid, _ in g.get("players", []):
            status = g.get("answers_by_user", {}).get(uid, {}).get(cat, {}).get("valid")
            if status is None:
                return False
    return True
...
# ---------- سایر handlers مانند lobby_button_handler, pick_category_handler, handle_group_message,
# manual_ok_handler, manual_no_handler مشابه نسخه قبلی هستند
# اما در پایان ثبت پاسخ‌ها باید اضافه شود:
# if is_round_complete(chat_id):
