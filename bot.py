...
async def efstart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("لطفاً دیگران را دعوت کنید و سپس «شروع دور» را بزنید.")
...
async def lobby_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "join":
        ...
        await context.bot.send_message(
            ...
            text=(
                "📖 *راهنما:*\n"
                "1. هر کسی می‌تواند /efstart را اجرا و سپس «شروع دور» را بزند.\n"
                ...
                "4. جواب‌های ناشناخته توسط ربات به خود شما جهت بررسی ارسال می‌شود.\n"
            ),
            parse_mode="Markdown"
        )
...
# ---------- ادامه هندلرها مانند pick_category_handler، handle_group_message، manual_ok_handler، manual_no_handler، finish_game، end_round_timeout و دستورات /score و /leave ----------
...
def main():
    ...
    # سایر هندلرها (pick_category_handler, manual_ok_handler, manual_no_handler, handle_group_message) باید اضافه شوند
