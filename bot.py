...
def build_lobby_text(chat_id: int) -> str:
    ...
    if not players:
        ...
        for i, (_, name) in enumerate(players, start=1):
            text += f"{i}. {name}\n"
    text += "\n━━━━━━━━━━━━\n"
    text += "➕ ورود به بازی\n🚀 کسی که ربات را صدا می‌زند، دور را شروع می‌کند\n"
    return text
...
async def efstart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    g["owner"] = user.id  # کسی که ربات را صدا می‌زند
...
async def lobby_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "join":
        ...
        await context.bot.send_message(
            ...
            text=(
                "📖 *راهنما:*\n"
                "1. با زدن /efstart دور را شروع کنید.\n"
                ...
                "4. جواب‌های ناشناخته توسط ربات برای caller جهت بررسی ارسال می‌شود."
            ),
            parse_mode="Markdown"
        )
        ...
        caller = user.id  # کسی که ربات را صدا می‌زند
        g["active"] = True
        ...
        await context.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🗂 انتخاب دسته", callback_data=f"open_catkbd:{chat_id}:{caller}")]
            ]),
            parse_mode="Markdown"
        )
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in valid_set:
        ...
        if ok:
            ...
            caller = g.get("owner")
            await context.bot.send_message(chat_id=chat_id, text=f"⚠️ پاسخ {user.full_name} نیاز به بررسی دارد؛ caller بررسی کند.")
